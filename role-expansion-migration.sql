...
-- 6. Auto-generate employee_id for existing staff
UPDATE profiles
SET employee_id = 'EMP-' || LPAD(ROW_NUMBER() OVER (ORDER BY created_at)::TEXT, 4, '0')
WHERE employee_id IS NULL AND role NOT IN ('client');

SELECT 'Role expansion migration complete!' AS status;
