...
export default function PricingClient({ plans }: { plans: any[] }) {
  ...
  const formatPrice = (priceStr: string | null | undefined) => {
    if (!priceStr) return "";
    // Remove commas for numeric check, but preserve original string if it has non-numeric parts like "+"
        ...
        <div className="container mx-auto px-4">
          {(!plans || plans.length === 0) ? (
            <div className="text-center py-20 bg-white border border-[#E2E8F0] rounded-2xl">
              <h2 className="text-xl font-bold text-[#0F172A] mb-2" style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}>No Pricing Plans Available</h2>
              <p className="text-[#64748B] text-sm text-center">Please check back later or contact us for a custom quote.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 flex-wrap justify-center">
              {plans.map((plan) => (
                <div
                  key={plan.id}
                  ...
                  } transition-all max-w-[400px] w-full`}
                >
                  {plan.is_recommended && (
                  ...
                  <p className="text-[#64748B] text-xs mb-5 min-h-[48px] line-clamp-3">{plan.description}</p>
                  ...
                  <ul className="space-y-2 mb-8 flex-1">
                    {getFeatures(plan.features).map((f: string, i: number) => (
