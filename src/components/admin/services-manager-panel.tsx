...
import { getServices, createService, updateService, deleteService } from "@/app/services-actions";

export default function ServicesManagerPanel() {
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({
    name: "", category: "", icon_name: "", tech_stack: "", 
    description: "", benefits: "", is_active: true, display_order: 0
  });
  ...
  const handleEdit = (item: any) => {
    setForm({
      name: item.name, category: item.category || "", icon_name: item.icon_name || "",
      tech_stack: item.tech_stack || "", description: item.description || "",
      benefits: item.benefits || "", is_active: item.is_active, display_order: item.display_order || 0
    });
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-sm text-[#64748B] mt-1">Add, edit, and manage your service offerings displayed on the website.</p>
          </div>
          <button
            onClick={() => {
              setEditingId(null);
              setForm({ name: "", category: "", icon_name: "", tech_stack: "", description: "", benefits: "", is_active: true, display_order: 0 });
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-[#CCFBF1] p-6 border-l-4 border-l-[#0D9488]">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#0F172A] mb-1.5">Service Name</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[#0F172A] mb-1.5">Icon Name (lucide-react)</label>
              <input type="text" value={form.icon_name} onChange={e => setForm({...form, icon_name: e.target.value})} placeholder="e.g. Globe, Code2, Smartphone" className="w-full px-4 py-3 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#0D9488]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#0F172A] mb-1.5">Technologies (comma-separated)</label>
              <input type="text" value={form.tech_stack} onChange={e => setForm({...form, tech_stack: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#0D9488]" />
            </div>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-[#0F172A] mb-1.5">Benefits (newline-separated)</label>
              <textarea rows={3} value={form.benefits} onChange={e => setForm({...form, benefits: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-[#E2E8F0] text-sm focus:outline-none focus:border-[#0D9488] resize-none" />
            </div>
            <div className="flex items-center gap-3">
              ...
              <label htmlFor="is_active" className="text-sm font-medium text-[#0F172A]">Active Service</label>
            </div>
            <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#E2E8F0] bg-[#F8FAFC]">
                  <th className="text-left py-3 px-4 text-xs font-semibold text-[#64748B] uppercase tracking-wider">Service Name</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-[#64748B] uppercase tracking-wider">Category</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-[#64748B] uppercase tracking-wider">Technologies</th>
              ...
              <tbody>
                {items.map((item, i) => (
                  <tr key={item.id || i} className="border-b border-[#F8FAFC] hover:bg-[#F8FAFC] transition-colors">
                    <td className="py-3 px-4 text-[#0F172A] font-medium">{item.name}</td>
                    <td className="py-3 px-4 text-[#64748B]">{item.category}</td>
                    <td className="py-3 px-4 text-[#64748B]">{item.tech_stack}</td>
                ...
                {items.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-6 text-gray-500">No services found.</td></tr>
                )}
              </tbody>
            </table>
          )}
